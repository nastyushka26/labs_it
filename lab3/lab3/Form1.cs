...
namespace lab3
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            // подписываем обработчиков на события
        ...
        // функция, в которой шифруется и дешифруется файл
        ...
        // алгоритм Евклида для нахождения просто НОД   - вместо него использую расширенный
        //public int gcd(int a, int b)                    он же все равно уже есть
        //{
        //    while (a != 0 && b != 0)
        //    {
        //        if (a > b)
        ...
        public bool check_fields()
        {
            if (tabControl1.SelectedTab == tbpgEncipher)
            {
                ...
                //if (gcd(d, fi_x) != 1)
                x1y1d1 = euclideX(d, fi_x); // x1y1d1 - массив с результатами алгоритма РАЕ
                //if (euclideX(d, fi_x)[2] != 1)  // в euclideX(d, fi_x)[2] - нод(d, fi_x)
                if (x1y1d1[2]!=1) // d1 = нод(fi_x, d)
        ...
        {
            int e;
            int[] result_euclide = euclideX(fi_x, d);
            e = result_euclide[1]; // 0 - x1, 1 - y1, 2 - d1
            if (e < 0)
                e += fi_x;
        ...
        public int[] euclideX(int a, int b)
        {
            ...
            // в d1 - НОД = 1, y1 - е
            return [x1, y1, d1];  // мультипликативное инверсное e
        }
